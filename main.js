...
console.log(
  ...
  "https://git.jacobtye.dev/Starkiller645/wolf-backend".bold.cyan,
  "┃"
);
console.log(
  "┃ WOLF was created by",
  "Tallie Tye".bold.yellow,
  "@",
  "https://jacobtye.dev".bold.cyan,
  "                         ┃"
);
console.log(
  "┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n"
...
const manifest_list = [
  rq_recent.manifest,
  rq_upcoming.manifest,
  rq_nextclash.manifest,
  rq_summoners.manifest,
  rq_bans.manifest,
  rq_picks.manifest,
  rq_livegame.manifest,
];
let manifest = [].concat(...manifest_list);
const manifest_dirs_list = [
  rq_recent.manifestdirs,
  rq_upcoming.manifestdirs,
  rq_nextclash.manifestdirs,
  rq_summoners.manifestdirs,
  rq_bans.manifestdirs,
  rq_picks.manifestdirs,
  rq_livegame.manifestdirs,
];
let manifestdirs = [].concat(...manifest_dirs_list);
const required_list = [
  rq_recent.required,
  rq_upcoming.required,
  rq_nextclash.required,
  rq_summoners.required,
  rq_bans.required,
  rq_picks.required,
  rq_livegame.required,
];
...
function check_manifest() {
  ...
  for (var item of required) {
    if (!fs.existsSync(item))
      console.err(
        "[err]".bold.red,
        'Could not find required file: "' + item + '"'
      );
  }
...
function clear_cache() {
  ...
  if (fs.existsSync("./team/summoners")) {
    ...
    console.err(
      "[/cachecheck]".bold.grey,
      "Error".bold.red + "couldn't find summoners directory"
    );
    return false;
  }
  var summoners = JSON.parse(fs.readFileSync("./config.json"))["team_members"];
  ...
  for (var summoner of summoner_cache) {
    if (!summoners.includes(summoner)) {
      fs.rmSync("./team/summoners/" + summoner, {
        recursive: true,
        force: true,
      });
    }
  ...
  if (cache_invalid) {
    if (fs.existsSync("./store/playercache.json"))
      fs.rmSync("./store/playercache.json");
...
function rebuild_cache(resolve, reject) {
  if (!fs.existsSync(cachefile)) {
    console.log(
      "[/cachecheck]".bold.grey,
      "Cachefile not found, forcing cache rebuild..."
    );
    ...
    for (var summoner of summoners) {
      ...
      lol.get("euw1", "summoner.getBySummonerName", summoner).then((data) => {
...
cache_cleared_promise.then(() => {
  ...
  setup_timers();
  express_app.listen(3080, () =>
    console.log("[/main]".bold.magenta, "Listening on port", "3080".magenta)
  );
});
